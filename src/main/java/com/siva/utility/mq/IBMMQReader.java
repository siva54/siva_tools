...
public class IBMMQReader {
	private static String[] ENV = new String[] { "PROD_US_NDM_1", "PROD_US_NDM_2", "PROD_UK_NDM_1", "PROD_UK_NDM_2",
			"PROD_CA_NDM_1", "PROD_CA_NDM_2", "PROD_HODM_1", "PROD_HODM_2" };
	private static final DataDumpStrategy DUMP_STRATEGY = DataDumpStrategy.SINGLE_FILE;

	private static final String EXTENSION = ".xml";

	@SuppressWarnings("unchecked")
	private static final String SINGLE_FILE_NAME = StringUtils.join("data", Constants.UNDERSCORE_DELIMITER,
	...
	public void process(String environment, QueueType queueType) throws Exception {
		...
		File destinationFolder = new File(
				StringUtils.join(applicationSettings.getProperty(Constants.MQ_DESTINATION_FOLDER),
		...
		MQQueue destQueue = qMgr.accessQueue(mqQueue, openOptions, null, null, null);
		...
		while (true) {
			...
			try {
				...
				case SINGLE_FILE:
					switch (queueType) {
					case BO:
						FileUtils.write(new File(destinationFolder, SINGLE_FILE_NAME),
								StringUtils.join(SINGLE_FILE_BO_PREFIX, new String(b), SINGLE_FILE_BO_SUFFIX),
								Charset.defaultCharset(), true);
						break;
					case BAD:
						FileUtils.write(new File(destinationFolder, SINGLE_FILE_NAME),
								StringUtils.join(SINGLE_FILE_BAD_PREFIX, new String(b), SINGLE_FILE_BAD_SUFFIX),
								Charset.defaultCharset(), true);
						break;
					}
					break;
				case INDIVIDUAL_FILES:
					FileUtils.writeByteArrayToFile(new File(destinationFolder, StringUtils.join(count, EXTENSION)), b,
							false);
					break;
				}
